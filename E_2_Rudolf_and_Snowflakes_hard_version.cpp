...
ll powss(ll a, ll p){if(p==0ll) return 1;if(p==1ll) return a;else if(p&1ll) {return a*powss(a*a,p/2);}else {return powss(a*a,p/2);}}
/*BS
    cin >> y;
    bool found = false;
    l = 0;
...
void solve()
{
    int n;
    cin >> n;
    for(ll i = 2;i <= 64;i++)//i ke k er power hishebe nibo
    {
        ll l = 2,r = 1e9+10;
        while(l < r-1)
        {
            ll m = (l+r)/2;//m holo k
            __int128 val = 1,s = 1;//__int128 is a 128 bit integer.it will run only in version 64 compiler
            for(ll power = 1;power<=i;power++)
            {
                s *= m;
                val += s;
                if(val > n)
