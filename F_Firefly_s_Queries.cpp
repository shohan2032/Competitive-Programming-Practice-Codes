...
void solve()
{
  ...
  */ 
    ...
    while(q--)
    {
        int l,r;
        cin>> l >> r;
        int lr = rotation_num(l,n),rr = rotation_num(r,n);
        l = index(l,lr,n);
        r = index(r,rr,n);
        // cout << lr << " " << l << " " << rr << " " << r << endl;
        if(lr == rr)
        {
            ...
            int left_segment_right_index = index(n*(lr+1),lr,n);
            int right_segment_left_index = index(n*rr + 1,rr,n);
            int ans = (rr-lr-1)*v[n-1];
            if(l-1 >= 0)
                ans += v[left_segment_right_index]-v[l-1];
            else
                ans += v[left_segment_right_index];
            if(right_segment_left_index-1 >= 0)
                ans += v[r]-v[right_segment_left_index-1];
            else
                ans += v[r];
            cout << ans << endl;
        }
