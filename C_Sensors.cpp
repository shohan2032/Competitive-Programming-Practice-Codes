...
{
    ch[i][j] = '*';
                if(i-1 >= 1 and j-1 >= 1 and ch[i-1][j-1] == '#')
                    dfs(i-1,j-1);
                if(i-1 >= 1 and j+1 <= c and ch[i-1][j+1] == '#')
                    dfs(i-1,j+1);
                if(i+1 <= r and j-1 >= 1 and ch[i+1][j-1] == '#')
                    dfs(i+1,j-1);
                if(i+1 <= r and j+1 <= c and ch[i+1][j+1] == '#')
                    dfs(i+1,j+1);
                if(i+1 <= r and ch[i+1][j] == '#')
                    dfs(i+1,j);
                if(i-1 >= 1 and ch[i-1][j] == '#')
                    dfs(i-1,j);
                if(j+1 <= c and ch[i][j+1] == '#')
                    dfs(i,j+1);
                if(j-1 >= 1 and ch[i][j-1] == '#')
                    dfs(i,j-1);
}
void solve()
{
  ...
  */ 
    ...
    while(!dq.empty())
    {
        if(ch[dq.front().first][dq.front().second] == '#')
        {
            ans++;
