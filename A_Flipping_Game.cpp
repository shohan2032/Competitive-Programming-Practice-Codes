...
void solve()
{
    ...
    for(int i = 1;i <= n;i++)
    {
        ...
        {
            int cnt = 0;
            cnt += arr[i-1];//1 to i-1 koita 1 ache
            cnt += arr[n]-arr[j];//j+1 to n koita 1 ache
            int x = j-i+1;//i to j total koita number
            int y = arr[j]-arr[i-1];//i to j koita 1 ache
            cnt += abs(x-y);//i to j total koita 0 ache jei gula flip hoiye 1 hobe
            ans = max(ans,cnt);
        }
    }
    cout << ans << endl;
