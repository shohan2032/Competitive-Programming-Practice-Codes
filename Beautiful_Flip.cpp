...
*/
int fun(int a,int b,int ans,int k,int x)
{
        int komate_hobe = a-ans;
        komate_hobe = min(komate_hobe,k);
        komate_hobe = min(komate_hobe,x);
        a -= komate_hobe;
        b += komate_hobe;
        return min(a,b);
}
  ...
  */ 
    ...
    for(int i = 0;i < n;)
    {
        ...
        if(ch == '1')
            ...
            z = max(z,cnt),zero += cnt;
    }
    int a = one,b = zero;
    // cout << o << " " << one << " " << z << " " << zero << endl;
    int ans = n/2,an = 0;
    if(one == zero)
    {
        cout << ans << endl;
    ...
    if(one  > zero)
    {
        // an = max(ans,fun(one,zero,ans,k,o));
        int komate_hobe = ans-zero;
        komate_hobe = min(komate_hobe,one-ans);
        komate_hobe = min(komate_hobe,k);
        komate_hobe = min(komate_hobe,o);
        one -= komate_hobe;
        zero += komate_hobe;
        // an = max(fun(zero,one,ans,k,z);
    }
    else
    {
        int komate_hobe = ans-one;
        komate_hobe = min(komate_hobe,zero-ans);
        komate_hobe = min(komate_hobe,k);
        komate_hobe = min(komate_hobe,z);
        zero -= komate_hobe;
        one += komate_hobe;
    }
    cout << max(min(one,zero),min(a,b)) << endl;
}   
