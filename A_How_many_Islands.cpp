...
{
    vis[x][y] = 1;
    if(x+1 <= r and !vis[x+1][y] and g[x+1][y] == '#')
        dfs(x+1,y);
    if(x-1 >= 1 and !vis[x-1][y] and g[x-1][y] == '#')
        dfs(x-1,y);

    if(y+1 <= c and !vis[x][y+1] and g[x][y+1] == '#')
        dfs(x,y+1);
    if(y-1 >= 1 and !vis[x][y-1] and g[x][y-1] == '#')
        dfs(x,y-1);
    
    if(x+1 <= r and y+1 <= c and !vis[x+1][y+1] and g[x+1][y+1] == '#')
        dfs(x+1,y+1);
    if(x+1 <= r and y-1 >= 1 and !vis[x+1][y-1] and g[x+1][y-1] == '#')
        dfs(x+1,y-1);

    if(x-1 >= 1 and y+1 <= c and !vis[x-1][y+1] and g[x-1][y+1] == '#')
        dfs(x-1,y+1);
    if(x-1 >= 1 and y-1 >= 1 and !vis[x-1][y-1] and g[x-1][y-1] == '#')
        dfs(x-1,y-1);
}
void solve()
{
  ...
  */ 
    ...
    while(!dq.empty())
    {
        if(!vis[dq.front().first][dq.front().second])
        {
            ans++;
            dfs(dq.front().first,dq.front().second);
        }
        dq.pop_front();
    }
    cout << ans << endl;
