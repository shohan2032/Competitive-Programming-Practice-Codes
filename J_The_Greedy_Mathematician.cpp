...
void solve()
{
  ...
  */ 
    ...
    for(int i = 1;i <= n;i++)
        cin>> vc[i];
    int pre_gc[n+5],suf_gc[n+5];
    pre_gc[1] = vc[1];
    suf_gc[n] = vc[n];
    for(int i = 2;i < n;i++)
        pre_gc[i] = gcd(pre_gc[i-1],vc[i]);
    for(int i = n-1;i >= 1;i--)
        suf_gc[i] = gcd(vc[i],suf_gc[i+1]);
    int max_gc = max(suf_gc[2],pre_gc[n-1]);
    for(int i = 2;i < n;i++)
        max_gc = max(max_gc,gcd(pre_gc[i-1],suf_gc[i+1]));
    int cnt = 0;
    if(suf_gc[2]==max_gc)
        cnt++;
    if(pre_gc[n-1] == max_gc)
        cnt++;
    for(int i = 2;i < n;i++)
        cnt += gcd(pre_gc[i-1],suf_gc[i+1]) == max_gc;
    cout << max_gc << " " << cnt << endl;
}   
