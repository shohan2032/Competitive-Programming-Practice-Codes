...
struct ST {
    ...
    inline void push(int n,int b,int e)//change this
    {
        if(lazy[n] == 1)
            return;
        t[n] = (t[n]*lazy[n])%mod;//parent node er value update
        if(b != e)
        {
            lazy[lc] = (lazy[lc]*lazy[n])%mod;//left child er lazy value update
            lazy[rc] = (lazy[rc]*lazy[n])%mod;//right child er lazy value update
        }
