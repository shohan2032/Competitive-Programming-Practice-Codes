...
void solve()
{
    ...
    while(q--)
    {
        ...
        if(ty == 1)
        {
            ...
            if(sum < x or (ind_of_one.empty() and x&1))
                cout << "NO" << endl;
            else
            {
                if(sum%2 == x%2)
                    cout << "YES" <<endl;//in that case sum-x will always be even
                else
                {
                    int ma = 0;
                    if(ind_of_one.size()){
                        auto it = ind_of_one.begin();
                        int f_o = *it;
                        auto itt = ind_of_one.end();
                        itt--;
                        int l_o = *itt;
                        int f_sum_array = query(1,1,n,1,l_o);
                        int l_sum_array = query(1,1,n,f_o,n);
                        ma = max(f_sum_array,l_sum_array);
                    }
                    if(ma>=x)
                        cout << "YES" << endl;
                    else
        ...
        else
        {
            ...
            cin>> ind >> val;
            if(a[ind] == 1)
                ind_of_one.erase(ind);
            if(val == 1)
                ind_of_one.insert(ind);
            a[ind] = val;
            upd(1,1,n,ind,val);
        }
    }
