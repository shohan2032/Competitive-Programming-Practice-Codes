...
void solve()
{
    ...
    for(int i = 0;i < s.size();i++)
    {
        if(s[i] == '+')
            ...
            size--;
            if(size < minNotSorted)
                minNotSorted = MXL;
            maxSorted = min(maxSorted,size);
        }
        else if(s[i] == '0')
        {
            if(maxSorted >= size or size < 2)
            {
                cout << "NO" << endl;
                return;
            }
            minNotSorted = min(minNotSorted,size);
        }
        else
        {
            if(minNotSorted <= size)
            {
                cout << "NO" << endl;
                return;
            }
            maxSorted = max(maxSorted,size);
        }
    }
    cout << "YES" << endl;
}   
