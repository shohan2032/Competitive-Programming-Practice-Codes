...
void solve()
{
    ...
    vl vc(n);
    fr(vc)cin >> i;
    ll s = accumulate(all(vc),0ll);
    ll prev_value_of_second_op = 0;
    set<ll>check_for_op_2;
    while(q--)
    {
        ll t;
        cin >> t;
        if(t == 1)
        {
            ...
            {
                if(check_for_op_2.find(ind-1) == check_for_op_2.end())
                {
                    check_for_op_2.insert(ind-1);
                    s -= prev_value_of_second_op;
                    s += x;
                    vc[ind-1] = x;
                    cout << s << endl;
                }
            ...
            s = n*x;
            if(prev_value_of_second_op != 0)
                check_for_op_2.clear();
            prev_value_of_second_op = x;
            cout << s << endl;
        }
    }
}   
