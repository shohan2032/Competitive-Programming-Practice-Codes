...
void dfs(int u,int v)
{
    ...
    // cout << u << " " << v << " " << cnt << " " << candy << endl;
    if(ch[u][v] == 'G')
        goal = true;
    if(ch[u][v] == 'o' and goal == false)
        candy++;
    else if(ch[u][v] == 'o' and goal)
        can++;
    ch[u][v] = '#';

    if(u+1 <= r and ch[u+1][v] != '#')   
        cnt++,dfs(u+1,v);
    if(u-1 >= 1 and ch[u-1][v] != '#')
        cnt++,dfs(u-1,v);
    if(v+1 <= c and ch[u][v+1] != '#')   
        cnt++,dfs(u,v+1);
    if(v-1 >= 1 and ch[u][v-1] != '#')
        cnt++,dfs(u,v-1);
}
void solve()
{
