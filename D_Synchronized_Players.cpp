...
void dfs(int x1,int y1,int x2,int y2,int cnt)
{
    ...
    vis1[x1][y1] = true;
    vis2[x2][y2] = true;
    if(x1+1 <= n and !vis1[x1+1][y1] and g[x1+1][y1] == '.')
    {
        if(x2+1 <= n and g[x2+1][y2] == '.')
            dfs(x1+1,y1,x2+1,y2,cnt+1);
        else
            dfs(x1+1,y1,x2,y2,cnt+1);
    }

    if(x1-1 >= 1 and !vis1[x1-1][y1] and g[x1-1][y1] == '.')
    {
        if(x2-1 >= 1 and g[x2-1][y2] == '.')
            dfs(x1-1,y1,x2-1,y2,cnt+1);
        else
            dfs(x1-1,y1,x2,y2,cnt+1);
    }

    if(y1+1 <= n and !vis1[x1][y1+1] and g[x1][y1+1] == '.')
    {
        if(y2+1 <= n and g[x2][y2+1] == '.')
            dfs(x1,y1+1,x2,y2+1,cnt+1);
        else
            dfs(x1,y1+1,x2,y2,cnt+1);
    }

    if(y1-1 >=1 and !vis1[x1][y1-1] and g[x1][y1-1] == '.')
    {
        if(y2-1 >=1 and g[x2][y2-1] == '.')
            dfs(x1,y1-1,x2,y2-1,cnt+1);
        else
            dfs(x1,y1-1,x2,y2,cnt+1);
    }

    if(x2+1 <= n and !vis2[x2+1][y2] and g[x2+1][y2] == '.')
    {
        if(x1+1 <= n and g[x1+1][y1] == '.')
            dfs(x1+1,y1,x2+1,y2,cnt+1);
        else
            dfs(x1,y1,x2+1,y2,cnt+1);
    }

    if(x2-1 >= 1 and !vis2[x2-1][y2] and g[x2-1][y2] == '.')
    {
        if(x1-1 >= 1 and g[x1-1][y1] == '.')
            dfs(x1-1,y1,x2-1,y2,cnt+1);
        else
            dfs(x1,y1,x2-1,y2,cnt+1);
    }

    if(y2+1 <= n and !vis2[x2][y2+1] and g[x2][y2+1] == '.')
    {
        if(y1+1 <= n and g[x1][y1+1] == '.')
            dfs(x1,y1+1,x2,y2+1,cnt+1);
        else
            dfs(x1,y1,x2,y2+1,cnt+1);
    }

    if(y2-1 >=1 and !vis2[x2][y2-1] and g[x2][y2-1] == '.')
    {
        if(y1-1 >=1 and g[x1][y1-1] == '.')
            dfs(x1,y1-1,x2,y2-1,cnt+1);
        else
            dfs(x1,y1,x2,y2-1,cnt+1);
    }
