...
int const N = 2e5+5;
int a[N],dp[N][2];
int fun(int ind,int state)//state 0 -> posSeg,state 1 -> negSeg
{
    if(dp[ind][state] != -1)
        return dp[ind][state];
    //base case
    ...
    {
        if(state == 0)
            dp[ind][state] = (a[ind] > 0?1:0);
        else
            dp[ind][state] = (a[ind] < 0?1:0);
        return dp[ind][state];
    }
    int cnt = 0;
    if(state == 0)//counting for posSeg
    {
        if(a[ind] > 0)
            cnt = fun(ind-1,0)+1;
        else
