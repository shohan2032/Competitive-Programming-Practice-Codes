...
void solve()
{
  ...
  */ 
    ...
    map<pair<int,int>,vi>moreThanOnce;
    for(int i = 1;i <= n;i++)
    {
        int x,y;
        cin>>x >> y;
        vc.pb(mp(x,y));
        mp[{x,y}] = i;
        moreThanOnce[{x,y}].pb(i);
    }
    fr(moreThanOnce)
    {
        if(i.second.size() > 1)
        {
            cout << i.second[0] << " " << i.second[i.second.size()-1] << endl;
            return;
        }
    }
    sort(all(vc),cmp);
    set<int>r;
    map<int,int>m;
    r.insert(vc[0].second);
    m[vc[0].second] = mp[vc[0]];
    for(int i = 1;i < n;i++)
    {
        int x = vc[i].second;
        ...
        if(it != r.end())
        {
            cout << mp[vc[i]] << " " << m[*it] << endl;
            return;
        }
        r.insert(vc[i].second);
        m[vc[i].second] = mp[vc[i]];
    }
    cout << -1 << " " << -1 << endl;
}   
