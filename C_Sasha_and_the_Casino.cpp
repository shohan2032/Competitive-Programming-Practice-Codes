...
void solve()
{
  ...
  */ 
    ...
    for(int i = 1;i <= x+1;i++)
    {
        int bet_to_regain_lose_plus_1 = lose/(k-1) + 1;//+1 karon o lose regain er sathe extra minimum 1 taka hoileo lav korte chaibe.else intital money barbe na.either kombe ba shoman thakbe.but he will intend to increase the initial money.so minimum 1 taka lav + lose regain korte koi taka bet korte hobe seita lagbe
        lose += bet_to_regain_lose_plus_1;
        if(lose > a)//lose jodi intital takar caite beshi hoi jai then he cannot increase his initial money
        {
            cout << "NO" << endl;
            return;
        }
