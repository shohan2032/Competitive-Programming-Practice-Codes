...
void solve()
{
    ll n,rem_cost,ins_cost;
    cin >> n >> rem_cost >> ins_cost;
    set<ll>st,sign_st;
    vl vc(n);
    fr(vc)
    {
    ...
    ll ans = MXL;
    ll cost_for_dup = (n-st.size())*rem_cost;
    if(st.find(1) == st.end())
    {
        st.insert(1);
        cost_for_dup += ins_cost;//for inserting 1;
    }
    while(!st.empty())
    {
        ll x = *st.begin();
        st.erase(x);
        sign_st.insert(x);
        ll r_c = st.size()*rem_cost;
        ll i_c = (x - sign_st.size())*ins_cost;
        ans = min(ans,r_c+i_c+cost_for_dup);
    }
    cout << ans << endl;
}   
