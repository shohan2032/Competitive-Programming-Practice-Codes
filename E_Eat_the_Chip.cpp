...
void solve()
{
  ...
  */ 
    ...
    if(d & 1)
    {
        int b_lm = max(1ll,by-(d/2));
        int b_rm = min(y,by+(d/2));
        if(abs(b_lm - ay) > ((d+1)/2) or abs(b_rm - ay) > ((d+1)/2))
            cout << "Draw" << endl;
        else
            cout << "Alice" << endl;
    }
    ...
    {
        int a_lm = max(1ll,ay-(d/2));
        int a_rm = min(y,ay+(d/2));
        if(abs(a_lm - by) > (d/2) or abs(a_rm - by) > (d/2))
            cout << "Draw" << endl;
        else
            cout << "Bob" << endl;
    }
