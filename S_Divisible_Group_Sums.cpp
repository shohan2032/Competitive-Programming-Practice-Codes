...
*/
int n,m,dp[201][25][11],d,k;
vi v;
int fun(int ind,int tar,int taken)
{
    if(tar == 0 and taken == 0)
        return 1;
    if(ind == n)
        return 0;
    if(dp[ind][tar][taken] != -1)
        return dp[ind][tar][taken];
    int ans = fun(ind+1,tar,taken);
    if(taken)
        ans += fun(ind+1,( ( ( (tar+v[ind]) %d) + d)% d),taken-1);
    return dp[ind][tar][taken] = ans;
}
void solve()
{
  /*
