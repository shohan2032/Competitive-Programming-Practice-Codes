...
void solve()
{
    ...
    vl vc(n+1),pos(n+1,-1);
    ll dp[n+1][2];
    dp[0][0] = 0;//dp[i][0] will hold the ans without getting vc[i]
    dp[0][1] = 0;//dp[i][1] will hold the ans getting the vc[i]
    ...
    for(int i = 1;i <= n;i++)
    {
        ...
        {
            int last = pos[vc[i]];
            ll x = max(dp[last-1][0],dp[last-1][1])+i-last+1;//2...[2...2]
            ll y = max(dp[last][1],dp[last][0])+i-last;//2...2[...2]
            // cout << x<< " " << y << endl;
            dp[i][1] = max(x,y);
        }
        pos[vc[i]] = i;
