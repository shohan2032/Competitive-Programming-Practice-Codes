...
void solve()
{
    ...
    else
    {
        ...
        for(int i = s;i <= x;i++)
        {
            ll front_remove = upper_bound(all(vc),i)-vc.begin();
            front_remove = n-(front_remove-1);
            ll back_remove = lower_bound(all(vc),i-s)-vc.begin();
            // cout << front_remove << " " << back_remove << endl;
            ans = min(ans,front_remove+back_remove);
        }
        cout <<  ans << endl;
    }
}   
