...
void solve()
{
    ...
    for(int i = 0;i < k;i++)
    {
        int x = l[i]-1;
        int y = r[i]-1;
        reverse(rev.begin()+x,rev.begin()+y+1);
    }
    int q;
    cin>> q;
    int mark[n+5]= {0};
    for(int i = 1;i <= q;i++)
    {
        int x;
        cin >> x;
        int ind = lower_bound(all(l),x)-l.begin();
        if(ind >= k or l[ind] > x)
            ind--;
        
        int left = min(x,l[ind]+r[ind]-x)-1;
        int right = max(x,l[ind]+r[ind]-x)-1;
...
/*Problem_link
    similar problem --> https://www.geeksforgeeks.org/reverse-given-range-of-string-for-m-queries/
*/