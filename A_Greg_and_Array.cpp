...
void solve()
{
    ...
    vl v(m+2);
    for(int i =1;i <= m;i++) 
        cin >> range[i].first >> range[i].second >> v[i];
    ll query_time[m+2]={0};
    for(int i = 0;i < k;i++)
    {
        int l,r;
    ...
    for(int i = 1;i <= m;i++)
        query_time[i] += query_time[i-1];
    // fr(query_time) cout << i << " ";
    ll diff[n+2] = {0};
    for(int i = 1;i <= m;i++)
    {
        if(query_time[i]){
            diff[range[i].first] += query_time[i]*v[i];
            diff[range[i].second+1] -= query_time[i]*v[i];
        }
    }
    for(int i = 1;i <= n;i++){
        diff[i] += diff[i-1];
