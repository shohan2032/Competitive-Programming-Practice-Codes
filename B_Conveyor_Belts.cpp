...
//  clockwise. For any natural k≥2
// , the matrix m2k
//  is obtained by adding to the matrix m2k−2
//  an outer layer forming a clockwise cycle.
void solve()
{
    //there will be n/2 levels.we just need to know in which level (x1,y1) and (x2,y2) belonged.
    //if the level of (x1,y1) is l1 and level of (x2,y2) is l2.then the ans is ABS(L1-L2)
    ll n,x1,y1,x2,y2;
    cin>> n>>x1 >> y1 >> x2 >> y2;
    if(x1 > n/2)
        x1 = n-(x1-1);
