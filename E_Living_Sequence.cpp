...
/*LB__UB
    l = -1,r = n;
        ...
        cout << r;
*/
string decToBase9(ll n)//as we need to remove digit 4 .so the availabelbase disit is (0,1,2,3,5,6,7,8,9).total base 9
{
    string s;
    while (n > 0)
    {
...
void solve()
{
    ll n;
    cin >> n;
    string s = decToBase9(n);
    for(int i = 0;i < s.size();i++)
    {
        if(s[i] > '3')
            cout << s[i]-'0' + 1;//as base9 will consider 9 digit .so it will give us the answer by decrementing one digit.
                                // question is ,which digit it will decrement.in this question the digit is 4.so whenever it gets a number greater than 3 ,it will give us a digit decrementing one compare to the decimal digit.
                                //so we need to increment one in that case as we have to give the answer in decimal
        else
            cout << s[i];
    }
    cout << endl;
