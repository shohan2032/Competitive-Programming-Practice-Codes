...
string big_mul(string a,string b){if(a.size() < b.size()) swap(a,b);string res;vector<string>vc;reverse(all(a));reverse(all(b));for(int i = 0;i < b.size();i++){string mulRes;int x = i;while(x--)mulRes += '0';x = b[i]-'0';int c = 0;for(int j = 0;j < a.size();j++){int y = x*(a[j]-'0');y += c;mulRes += '0'+(y%10);c = y/10;}
if(c)mulRes += '0'+c;reverse(all(mulRes));vc.pb(mulRes);}res = vc[0];for(int i = 1;i < vc.size();i++)res = big_add(res,vc[i]);return res;}//a*b
