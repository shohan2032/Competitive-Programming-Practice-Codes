  ...
  */ 
    ...
    for(int i = 0;i < 3;i++)
    {
        for(int j = 0;j < 3;j++)
        {
            for(int k = 0;k < 3;k++)
            {
                if(s[i].second != m[j].second and s[i].second != g[k].second)//sd != md and sd != gd
                {
                    if(m[j].second != g[k].second)//md != gd
                        ans = max(ans,s[i].first+m[j].first+g[k].first);
                }
            }
        }
    }
