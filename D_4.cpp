...
void solve()
{
  ...
  */ 
    int n;
    cin>> n;
    map<string,vector<string>>mp;//dis -> symptome
    cin.ignore();
    while(n--)
    {
        string dis;
    ...
    fr(mp)
    {
        double total_sym = i.second.size();
        double milse = 0,milenai;
        fr(i.second)
        {
            if(sy.count(i))
                milse++;
        }
        milenai = sy.size() - milse;
        double probability = milse/(total_sym+milenai);
        // cout << i.first << " " << total_sym << " " << milse << " " << milenai << " " << probability << endl;
        ans.pb(mp(i.first,probability));
    }
    sort(all(ans),cmp);
    fr(ans)
    {
