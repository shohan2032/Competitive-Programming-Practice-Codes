...
void solve()
{
  ...
  */ 
    ...
    fr(rr)
    {
        ...
        vi a = i.second;
        vi b = cc[i.first];
        partial_sum(all(i.second),i.second.begin());
        partial_sum(all(cc[i.first]),cc[i.first].begin());
        for(int j = 0;j < i.second.size()-1;j++)
        {
            int x = i.second.size()-1;
            ans += i.second[x]-i.second[j]-((x-j)*a[j]);
            ans += cc[i.first][x]-cc[i.first][j]-((x-j)*b[j]);
        }
    }
    cout << ans << endl;
}   
