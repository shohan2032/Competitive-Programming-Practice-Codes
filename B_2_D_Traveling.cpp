...
void solve()
{
  ...
  */ 
    ...
    for(int i = 1;i <= k;i++)
    {
      if(i!=b)
      {
        int x = abs(vc[i].first - vc[b].first);
        x += abs(vc[i].second - vc[b].second);
        if(i <= k and b <= k)
          x = 0;
        if(x < ans)
        {
    ...
    for(int i = 1;i <= k;i++)
    {
      int x = abs(vc[a].first - vc[i].first);
      x += abs(vc[a].second - vc[i].second);
      if(a <= k and i <= k)
        x = 0;
      int y= abs(vc[i].first - vc[f].first);
      y += abs(vc[i].second - vc[f].second);
      if(i <= k and f <= k)
        y = 0;
      an = min(an,x+y);
    }
    int x = abs(vc[a].first - vc[b].first) + abs(vc[a].second - vc[b].second);
    cout << min(x,an+ans) << endl;
}   
