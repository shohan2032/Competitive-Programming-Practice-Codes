...
void solve()
{
  ...
  */ 
    ...
    {
        int total_permu = fact[n];
        int cnt = mp[vc[n-1]-1];
        int bad = n-cnt-1;
        //bad permutaton hobe P(n,bad) = n! / (n-bad)
        //As we know,By choosing k distinct elements from n distinct elemnets we can have P(n,k) = n! / (n-k) permutation.
        //here k = bad,So
        //n-k will be n - (n-cnt-1) = cnt+1;
        bad = cnt+1;
        int bad_permutaion = binmul(total_permu,inverse(bad,mod),mod);
        cout << mo(total_permu - bad_permutaion , mod) << endl;
    }
}   
