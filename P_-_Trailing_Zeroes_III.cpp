...
int main()
{
    ...
    for(int i = 1;i <= t;i++)
    {
        ...
        {
            ll m = (l+r)/2;
            ll trailing_zero_in_m = num_trailing_zero(m);
            if(trailing_zero_in_m > x)
                r = m-1;
            else if(trailing_zero_in_m < x)
                l = m + 1;
            else
            {
                ans = m;
                r = m-1;// as we need the minimal number
            }
        }
        if(ans)
