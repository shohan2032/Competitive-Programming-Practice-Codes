...
void divideAndCon(string s,int ind)
{
    if(ind == s.size())
    {
        int num = stoi(s);
        int setBit = __builtin_popcount(num);
        ans.pb(mp(setBit,num));
        return;
    }
    if (s[ind] == '?') 
    {
...
/*Problem_link
    https://www.hackerrank.com/contests/srbd-code-contest-2024-round-1/challenges/not-number-theory
