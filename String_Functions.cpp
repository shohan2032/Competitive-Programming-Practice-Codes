...
int n;
int fun(int ind,int type)
{
    int l = 0,r = (type==1?n-ind+1:ind+1);
    while(l < r - 1)
    {
        int m = (l+r)/2;
        ...
        {
            if(type == 1)
                return h1.get_hash(0,m-1) == h1.get_hash(ind,ind+m-1);
            else    
                return h1.get_hash(0,m-1) == h1.get_hash(ind-m+1,ind);
        };
        if(ok(m))
            l = m;
        else
