...
void solve()
{
  ...
  */ 
    ...
    double l = 0.0,r = 1e9,l_ma = 0.0,r_mi = 1e9;
    for(int i = 1;i <= n;i++)
    {
        double a = vc[i-1].first;
        double b = vc[i-1].second;
        // if(a >= l_ma and a)
        // {
        //     // cout << a << endl;
        //     l_ma = a;
    ...
    {
        double m = (l+r)/2;
        auto ok = [&](double m)->int //return type bool
        {
            for(int i = 1;i <= n;i++){
                double t = ((double)(i))/m;
                // cout << t << " " << vc[i-1].first << " " << vc[i-1].second << endl;
                if(t < vc[i-1].first)
                    return 1;
                if(t > vc[i-1].second)
                    return 2;
...
int32_t main()
{
    ...
    cin.tie(NULL);
    cout.tie(NULL);
    run_with_stack_size(solve, 1024 * 1024 * 1024); // run with a 1 GiB stack
    // auto st = clock();
    int t = 1;
    cin >> t;
    for(int i = 1;i <= t;i++)
