...
*/
int bfs(map<int,int>snakeAndladder)
{
    int vis[200] = {};
    queue<int>pq;
    ...
    while(!pq.empty())
    {
        int cur = pq.front();
        pq.pop();
        for(int i = cur+1;i<= min(100ll,cur+6);i++)
        {
            int next = i;
            if(snakeAndladder.count(next))
