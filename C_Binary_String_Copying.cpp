...
void solve()
{
    ...
    for(int i = 0;i < n;i++)
    {
        ...
        if(s[i] == '1')
            f = 1;
        initially_sorted[i] = color;
    }
    // for(int i = 0;i < n;i++)
    //     cout << initially_sorted[i] << " ";
    // cout<< endl;
    ll prefix_for_one[n+5],suffix_for_zero[n+5];
    f = 0;
    prefix_for_one[0] = f;
    for(int i = 0;i < n;i++)
    {
        if(s[i] == '1')
            f = i+1;
        prefix_for_one[i+1] = f; 
    }
    f = n+1;
    suffix_for_zero[n+1] = f;
    for(int i = n-1;i >= 0;i--)
    {
        if(s[i] == '0')
            f = i+1;
        suffix_for_zero[i+1] = f;
    }
    // for(int i = 0;i <= n;i++)
    //     cout << prefix_for_one[i] << " ";
    // cout << endl;
    // for(int i = 1;i <= n+1;i++)
    //     cout << suffix_for_zero[i] << " ";
    ...
    for(int i = 1;i<= m;i++)
    {
        ...
        if(initially_sorted[l-1] == initially_sorted[r-1])
            ans.insert(mp(-1,-1));
        else
        {
            l = prefix_for_one[l-1]+1;
            r = suffix_for_zero[r+1]-1;
            ans.insert(mp(l,r));
        }
    }
    cout << ans.size() << endl;
