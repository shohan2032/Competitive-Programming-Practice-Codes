...
void solve()
{
  ...
  */ 
    ...
    fr(vc)cin>>i;
    int s = -1e18;
    int max_sub_array_sum =  -1e18;
    fr(vc)
    {
        s += i;
        s = max(s,i);
        max_sub_array_sum = max(max_sub_array_sum,s);
    }
    // cout << max_sub_array_sum << endl;
    if(max_sub_array_sum <= 0)
        cout << mo(accumulate(all(vc),0ll) , mod) << endl;
    else
    {
        int val = binmul(max_sub_array_sum, (binpow(2,k,mod) - 1), mod);
        val += mo(accumulate(all(vc),0ll),mod);
        cout << mo(val,mod) << endl;
    }
}   
