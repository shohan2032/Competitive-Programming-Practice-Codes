...
void solve()
{
    ...
    fr(mp)
    {
        ...
        if(i.first > 0)
        {
            int pre = i.first-1;
            int suf = m-i.first;
            int niche = ind-1-mp[-1]-mp[-2];
            int upore = sz-ind;
            ans = max(ans,(min(pre,o+niche)+min(suf,t+upore))+1);
        }
    }
    if(t){
        int sudu_2_er_khetre = sz-mp[-1]-mp[-2];
        sudu_2_er_khetre += t;
        ans = max(ans,sudu_2_er_khetre);
    }
    if(o)
    {
        int sudu_1_er_khetre = sz-mp[-1]-mp[-2];
        sudu_1_er_khetre += o;
        ans = max(ans,sudu_1_er_khetre);
    }
    cout << min(m,ans) << endl;
}   
