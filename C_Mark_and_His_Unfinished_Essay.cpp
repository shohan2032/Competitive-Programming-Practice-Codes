...
void solve()
{
  ...
  */ 
    ...
    string s;
    cin>> s;
    vector<pair<int,int>>seg,op;
    vi dif_starting_ending;
    int last_l = n;
    while(c--)
    {
        int l,r;
        cin>> l >> r;
        op.pb(mp(l,r));
        seg.pb(mp(last_l+1,last_l+1+(r-l)));
        last_l = seg.back().second;
        dif_starting_ending.pb(seg.back().first - l);
    }
    // fr(dif_starting_ending)cout << i << " ";
    while(q--)
    {
        int x;
        cin>> x;
