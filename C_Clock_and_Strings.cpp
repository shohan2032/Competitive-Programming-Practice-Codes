...
void solve()
{
  ...
  */ 
    int a,b,c,d;
    cin >> a >> b >> c >> d;
    if(min(a,b) < min(c,d) and max(a,b) > min(c,d) and min(a,b) < max(c,d) and max(a,b) < max(c,d))
        cout << "YES" << endl;
    else if(min(c,d) < min(a,b) and max(c,d) > min(a,b) and min(c,d) < max(a,b) and max(c,d) < max(a,b))
        cout << "YES" << endl;
    else
        cout << "NO" << endl;
}    
