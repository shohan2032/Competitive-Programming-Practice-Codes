...
void solve()
{
  ...
  */ 
    ...
    fr(v)
    {
        if(i.second >= 2)
        {
            int total = binmul(binmul(i.second,i.second-1,mod),inverse(2,mod),mod);
            total = binmul(total,i.first*i.first,mod);
            ans += total;
            ans %= mod;
        }
        // cout << i.first << " " << ans << " ";
        int pre_pro = binmul(s,binmul(i.first,i.second,mod),mod);
        ans += pre_pro;
        // cout << ans << endl;
        ans %= mod;
        int pre = binmul(i.first,i.second,mod);
        s += pre;
        s %= mod;
    }
    n = binmul(binmul(n,n-1,mod),inverse(2,mod),mod);
    // cout << ans << " " << n << endl;
    ans = binmul(ans,inverse(n,mod),mod);
    cout << ans << endl;
}   
