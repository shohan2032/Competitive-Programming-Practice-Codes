...
void solve()
{
  ...
  */ 
    ...
    map<int,int>cur_dp,new_dp;//lcm --> size
    for(int j = 0;j < n;j++)
    {
        new_dp[vc[j]] = max(new_dp[vc[j]],1ll);
        fr(cur_dp)
        {
            int new_lcm = lcm(i.first,vc[j]);
            new_dp[new_lcm] = max(new_dp[new_lcm],i.second+1);
        }
        cur_dp = new_dp;
    }
    set<int>st(all(vc));
