...
vector<int> kmp(vector<int> lps, string s, string p) {//TC->O(|s|+|p|)
    vector<int>ans;
...
map<int,priority_queue<int,vi,greater<int>>> pr;
map<int,priority_queue<int>>su;
void solve()
{
  /*
    1. Think Greedy
  ...
  */ 
    ...
    while(q--)
    {
      ...
      {
        pre += p[i];
        suf = p.substr(i+1,p.size()-(i+1));
        vector<int>lp;
        lp = build_lps(pre);
        vector<int>pre_ind = kmp(lp,s,pre);
        lp.clear();
        lp = build_lps(suf);
        vector<int>suf_ind = kmp(lp,s,suf);
        if(pre_ind.size())
          pr[pre.size()].push(pre_ind[0]);
        if(suf_ind.size())
          su[suf.size()].push(suf_ind.back());
      }
      fr(pr)
      {
        int pre_len = i.first;
