...
void solve()
{
    ...
    double ans = max(hypot(ax,ay),hypot(ax-px,ay-py));//A cover OP
    ans = min(ans,max(hypot(bx,by),hypot(bx-px,by-py)));//B cover OP
    ans = min(ans,max({hypot(ax,ay),hypot(bx-px,by-py),hypot(ax-bx,ay-by)*0.5}));//A cover O and B cover P
    ans = min(ans,max({hypot(bx,by),hypot(ax-px,ay-py),hypot(ax-bx,ay-by)*0.5}));//B cover O and A cover P
