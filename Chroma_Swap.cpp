...
void solve()
{
  ...
  */ 
    int n;
    cin>> n;
    vi a(n+5),b(n+5),c1(n+5);
    vector<multiset<int>>allCol(2*n + 5);
    bool mark[2*n + 5] = {};
    for(int i = 0;i < n;i++)
    {
        cin>> a[i];
    ...
    for(int i = 0;i < n;i++)
    {
        if(!mark[c1[i]])
        {
            ...
            auto it = lower_bound(all(allCol[c1[i]]),pre);
            if(it == allCol[c1[i]].end())
            {
                cout << "No" << endl;
                return;
            }
            pre = *it;
            allCol[c1[i]].erase(it);
        }
    }
    cout << "Yes" << endl;
}   
