...
struct ST {
    ...
    inline void push(int n,int b,int e)//change this
    {
        if(lazy[n] == 0)
            return;
        t[n] = max(t[n],lazy[n]);//parent node er value update
        if(b != e)
        {
            lazy[lc] = max(lazy[lc],lazy[n]);//left child er lazy value update
            lazy[rc] = max(lazy[rc],lazy[n]);//right child er lazy value update
        }
...
void solve()
{
    ...
    while(q--)
    {
        ...
        else
        {
            ...
            cin>> ind;
            ind++;
            cout << t.query(1,1,n,ind,ind) << endl;
        }
    }
}   
