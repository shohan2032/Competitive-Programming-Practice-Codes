...
void build(int node,int b,int e)//b theke e node er range
{
    ...
    t[node] = max(t[l],t[r]);//saving the sum of this node.Change this one according to question
}

int query(int node,int b,int e,int x,int ind)//query hocche i theke j(TC-->Log(N))
{
    if(t[node] < x) 
        return -1;
    if(e < ind)
    ...
    int r = node*2 + 1;
    int mid = (b+e)/2;
    int ans = query(l,b,mid,x,ind);
    if(ans == -1)
        ans = query(r,mid+1,e,x,ind);   
    return ans;
}
...
void upd(int node,int b,int e,int i,int x)//i index er value ke update kore x banaute hobe(TC-->log(n))
{
    ...
    upd(l,b,mid,i,x);
    upd(r,mid+1,e,i,x);
    t[node] = max(t[l],t[r]);//Change this one according to question
}
