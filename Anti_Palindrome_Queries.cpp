...
void solve()
{
  ...
  */ 
    ...
    partial_sum(all(one),one.begin());
    partial_sum(all(two),two.begin());
    partial_sum(all(three),three.begin());
    while(q--)
    {
        int l,r;
        cin >> l >> r;
        ...
        {
            int oneAche = one[r] - one[l-1];
            int twoAche = two[r] - two[l-1];
            int threeAche = three[r] - three[l-1];
            if(oneAche > len/2 or twoAche > len/2 or threeAche > len/2)
            {
                cout << "No" << endl;
                continue;
            }
