...
void solve()
{
  ...
  */ 
    ...
    // cout << endl;
    vpi x,y;
    int cur_v = aa[0].first,cur_cnt = aa[0].second;
    for(int i = 1;i < aa.size();i++)
    {
        if(aa[i].first != aa[i-1].first)
        {
            x.pb(mp(cur_v,cur_cnt));
            cur_v = aa[i].first;
            cur_cnt = aa[i].second;
        }
        else
    ...
    {
        if(bb[i].first != bb[i-1].first)
        {
            y.pb(mp(cur_v,cur_cnt));
            cur_v = bb[i].first;
            cur_cnt = bb[i].second;
        }
        else
