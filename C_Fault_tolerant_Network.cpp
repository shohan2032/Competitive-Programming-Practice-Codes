...
void solve()
{
  ...
  */ 
    ...
    int ans1= abs(a[0] - b[n-1]) + abs(b[0] - a[n-1]);

    int ans2 = abs(a[0] - b[0]) + abs(a[n-1] - b[n-1]);

    int ans3 = abs(a[0] - b[0]);
    int m1 = MXL,m2 = MXL;
    for(int i = 0;i < n;i++)
    {
        m1 = min(m1,abs(a[n-1] - b[i]));
        m2 = min(m2,abs(b[n-1] - a[i]));
    }
    ans3 += m1+m2;

    int ans4 = abs(a[n-1] - b[n-1]);
    m1 = MXL,m2 = MXL;
    for(int i = 0;i < n;i++)
    {
    ...
    {
        m1 = min(m1,abs(a[0] - b[i]));
        m2 = min(m2,abs(a[n-1] - b[i]));
        m3 = min(m3,abs(b[0] - a[i]));
        m4 = min(m4,abs(b[n-1] - a[i]));
    }
    int ans7 = m1+m2+m3+m4;
