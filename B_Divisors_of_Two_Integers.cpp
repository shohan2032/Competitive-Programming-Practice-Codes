...
void solve()
{
    ...
    fr(vc) cin >> i;
    sort(all(vc));
    set<int>div_of_big_one;
    int big_one = vc[n-1];
    cout << big_one << " ";
    for(int i = 1;i*i <= big_one;i++)
    {
        if(big_one % i == 0)
        {
            div_of_big_one.insert(i);
            div_of_big_one.insert(big_one/i);
        }
    }
    for(int i = n-1;i >= 0;i--)
    {
        if(div_of_big_one.find(vc[i]) == div_of_big_one.end() or vc[i] == vc[i-1])
        {
            cout << vc[i] << endl;
            return;
        }
