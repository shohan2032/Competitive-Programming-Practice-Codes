...
void solve()
{
  ...
  */ 
    ...
    {
        pair<int,int> pi1 = st1[i-1];
        pair<int,int> pi2 = st2[i-1];
        ...
        st1.pb(mp(a[ind1],p[ind1]));
        st2.pb(mp(a[ind2],p[ind2]));
    }
    int cnt = 0;
    for(int i=0;i < st1.size();i++)
    {
        bb = max(bb,cnt+(k-i)*st1[i].first);
        cnt += st1[i].first;
        // cout << bb << endl;
    }
    cnt = 0;
