...
const int N = 1e6 + 9, p1 = 137, p2 = 277, mod1 = 1e9 + 7, mod2 = 1e9 + 9;//p2 = 281
...
pair<int,int> palindromic_sub(int n)//return the count of all substrings which are palindrome.Tc-->NlogN
{
    int max_len = 0;
    int ind = 0;
...
void solve()
{
    ...
    reverse(all(s));
    h2.build(s);
    pair<int,int> pi = palindromic_sub(s.size());
    cout << ss.substr(pi.first,pi.second) << endl;
}   
