...
void solve()
{
  ...
  */ 
    ...
    fr(vc)
    {
        if(mp.count(mp(i,0)) and mp.count(mp(i,1)))
            ans += n-2;
        if(mp.count(mp(i,0)) and mp.count(mp(i+1,1)) and mp.count(mp(i+2,0)))
            ans++;
        if(mp.count(mp(i,1)) and mp.count(mp(i+1,0)) and mp.count(mp(i+2,1)))
            ans++;
    }
    cout << ans << endl;
