...
void solve()
{
    ...
    sort(all(vc),[&](const pair<ll,ll> &a1,const pair<ll,ll> &a2){return a1.second < a2.second;});
    // fr(vc)
    //     cout << i.first << " "  << i.second << endl;
    for(int i = 0;i < 2*n;i++)
        dq.pb(mp(vc[i].first,vc[i].second));
    sort(all(dq),[&](const pair<ll,ll> &a1,const pair<ll,ll> &a2){return a1.first < a2.first;});
    // fr(dq)
    //     cout << i.first << " "  << i.second << endl;
    ll an = 0;
    while(!dq.empty())
    {
        an +=  (dq.front().second + dq.back().second);
        ans.pb(mp(mp[dq.front().first],mp[dq.back().first]));
        dq.pop_front();
        dq.pop_back();
    }
    cout << an << endl;
