...
void solve()
{
  ...
  */ 
    ...
    if(n%2== 0)
    {
        ...
        fr(e)
            em = max(em,i.second);
        cout << (op-om)+(ep-em) <<endl;
    }
    else
    {
        vector<vector<int>>pre(n+6,vector<int>(26,0));
        vector<vector<int>>suf(n+6,vector<int>(26,0));
        int pre_ind[n+5] = {},suf_ind[n+5] = {};
        for(int i = 0;i < n;i++)
        {
            if(i - 2 >= 0)
            {
                pre_ind[i] = pre_ind[i-2];
                pre[i] = pre[i-2];
            }
            pre_ind[i]++;
            pre[i][s[i]-'a']++;
        ...
        for(int i = 0;i < n;i++)
        {
            ...
            for(int j = 0;j <= 25;j++)
            {
                int j_ache = 0,total_ind = 0;
                if(i-1>=0)
                    j_ache += pre[i-1][j],total_ind += pre_ind[i-1];
                if(i+2 < n)
                    j_ache += suf[i+2][j],total_ind += suf_ind[i+2];
                bad1 = min(bad1,total_ind-j_ache);
            }
            for(int j = 0;j <= 25;j++)
            {
                int j_ache = 0,total_ind = 0;
                if(i-2>=0)
                    j_ache += pre[i-2][j],total_ind += pre_ind[i-2];
                if(i+1 < n)
                    j_ache += suf[i+1][j],total_ind += suf_ind[i+1];
                bad2 = min(bad2,total_ind-j_ache);
            }
            ans = min(ans,bad1+bad2+1);
        }
        cout << ans << endl;
    }
}   
