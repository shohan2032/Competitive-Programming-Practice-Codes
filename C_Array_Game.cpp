...
void solve()
{
  ...
  */ 
    ...
    for(int i = 0;i < n-1;i++)
    {
        ...
        {
            int x = vc[j]-vc[i];
            int up = upper_bound(all(vc),x-1)-vc.begin();//or shoman ba tar caite boro ta return korbe
            int lo = lower_bound(all(vc),x)-vc.begin();
            if(lo)lo--;//x er choto ta return korbe
            mn = min(mn,abs(x-vc[up]));
            mn = min(mn,abs(x-vc[lo]));
        }
    }
    cout << mn <<endl;
}   
