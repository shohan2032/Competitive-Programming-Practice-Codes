...
int const N = 3e5+9;
int siz[N],parent[N];
deque<pair<int,int>> closed_roads;
...
void solve()
{
    ...
    if(closed_roads.size() >= total_com.size()-1)
    {
        cout << total_com.size()-1 << endl;
        for(int i = 1;i < total_com.size();i++)
        {
            int x = closed_roads.front().first;
            int y = closed_roads.front().second;
            int u = total_com[0];
            int v = total_com[i];
            cout << x << " " << y << " " << u << " " << v << endl;
            closed_roads.pop_front();
        }
    }
    else
        cout << 0 << endl;
